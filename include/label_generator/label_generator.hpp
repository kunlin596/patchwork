#ifndef INCLUDE_LABEL_GENERATOR_LABEL_GENERATOR_HPP_
#define INCLUDE_LABEL_GENERATOR_LABEL_GENERATOR_HPP_
...
void filter_by_sor(const pcl::PointCloud<PointType> &src,
                   pcl::PointCloud<PointType> &filtered,
                   int num_neighbor_pts = 20,
                   float std_multiplier = 5.0) {
...
template <typename PointType>
void save_ground_label(const std::string abs_dir,
                       const int frame_num,
  ...
  std::vector<uint32_t> labels(num_cloud_raw,
                               EST_NON_GROUND);  // 0: Non ground points
...
#endif  // INCLUDE_LABEL_GENERATOR_LABEL_GENERATOR_HPP_
